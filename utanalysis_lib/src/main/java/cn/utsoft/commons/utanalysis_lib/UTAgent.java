...
public class UTAgent {
    ...
    static {
        HandlerThread localHandlerThread = new HandlerThread("UTAgent");
    ...
    private static void init(Context context) {
        ...
        UTAgent.postHistoryLog();
        UTAgent.postClientData();
        
        UTAgent.onError();
        CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "Call init();BaseURL = " + UTConstants.BASE_URL);
...
//	}
    ...
    public static void init(Context context,String baseUrl, String appkey) {
        if (appkey.length() == 0 || baseUrl.length() ==0) {
            CobubLog.e(UTConstants.LOG_TAG, UTAgent.class, "appkey and baseUrl are required");
            return;
        }
        UTConstants.BASE_URL = baseUrl;
        updateContent(context);
        INIT = true;
    ...
    public static void setSessionContinueMillis(Context context, long interval) {
        if (!INIT) {
            CobubLog.e(UTConstants.LOG_TAG, UTAgent.class, "sdk is not init!");
            return;
        }
        updateContent(context);
        CobubLog.i(UTConstants.LOG_TAG, UTAgent.class,
                "setSessionContinueMillis = " + String.valueOf(interval));
        if (interval > 0) {
            UTConstants.kContinueSessionMillis = interval;
            SharedPrefUtil spu = new SharedPrefUtil(contextWR.get());
            spu.setValue("SessionContinueMillis", interval);
        } else {
            CobubLog.e(
                    UTConstants.LOG_TAG, UTAgent.class,
    ...
    public static void setAutoLocation(boolean isLocation) {
        ...
        UTConstants.mProvideGPSData = isLocation;
        CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "setAutoLocation = " + String.valueOf(isLocation));
    }
    ...
    static void postClientData() {
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                if (isFirst) {
                    CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "Start postClientdata thread");
    ...
    static void postHistoryLog() {
        CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "postHistoryLog");
    ...
    public static void bindUserIdentifier(Context context,
        ...
        updateContent(context);
        CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "Bind user identifier");
    ...
    public static void onResume(Context context) {
        ...
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "Call onResume()");
    ...
    public static void onFragmentResume(Context context,
        ...
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "Call onFragmentResume()");
                if (usinglogManager == null)
                    usinglogManager = new UsinglogManager(contextWR.get());
    ...
    public static void onPause(Context context) {
        ...
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "Call onPause()");
                if (usinglogManager == null)
                    usinglogManager = new UsinglogManager(contextWR.get());
    ...
    static void onError() {
        ...
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "Call onError()");
    ...
    public static void onError(Context context, final String errorType, final String errorinfo) {
        ...
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "Call onError(context,errorinfo)");
    ...
    public static void postTags(Context context, final String tags) {
        ...
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "Call postTags()");
    ...
    public static void onEvent(Context context, final String event_id) {
        ...
        Thread thread = new Thread(new Runnable() {
            public void run() {
                CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "Call onEvent(context,event_id)");
    ...
    public static void onEvent(Context context, final String event_id,
        ...
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "Call onEvent(event_id,acc)");
    ...
    public static void onEvent(Context context, final String event_id,
        ...
        if (event_id == null || event_id.length() == 0) {
            CobubLog.e(UTConstants.LOG_TAG, UTAgent.class, "Valid event_id is required");
        }
        if (acc < 1) {
            CobubLog.e(UTConstants.LOG_TAG, UTAgent.class, "Event acc should be greater than zero");
        }

        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "Call onEvent(event_id,label,acc)");
    ...
    public static void onEvent(Context context, final String event_id,
        ...
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "Call onEvent(context,event_id,label,acc)");
                EventManager em = new EventManager(contextWR.get(), event_id, label,
                        "1", json);
                em.postEventInfo();
            }
        ...
        Runnable postEventRunnable = new Runnable() {
            ...
            public void run() {
                ...
                EventManager em = new EventManager(contextWR.get(),
                        UTAgent.EVENT_DEFAULT, label, value);
                em.postEventInfo();
            }
        };
    ...
    public static void update(Context context) {
        ...
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "Call update()");
    ...
    public static void setDebugEnabled(boolean isEnableDebug) {
        if (!INIT) {
            Log.e(UTConstants.LOG_TAG, "sdk is not init!");
        }
        UTConstants.DebugEnabled = isEnableDebug;
    }
    ...
    public static void updateOnlineConfig(Context context) {
        ...
        final Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "Call updaeOnlineConfig");
    ...
    public static void setUpdateOnlyWifi(boolean isUpdateonlyWifi) {
        ...
        UTConstants.mUpdateOnlyWifi = isUpdateonlyWifi;
        if (contextWR.get() == null) {
            CobubLog.e(UTConstants.LOG_TAG, UTAgent.class,
                    "UTAgent.context is null,please call init() before ");
            return;
        }
        SharedPrefUtil spu = new SharedPrefUtil(contextWR.get());
    ...
    public static void setDefaultReportPolicy(Context context,
        ...
        updateContent(context);
        UTConstants.mReportPolicy = sendPolicy;
        int type = 1;
    ...
    public static void postWebPage(final String pageName) {
        ...
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "Call postWebPage()");
                if (usinglogManager == null)
                    usinglogManager = new UsinglogManager(contextWR.get());
    ...
    public static void setPostIntervalMillis(Context context, long interval) {
        ...
        CobubLog.i(UTConstants.LOG_TAG, UTAgent.class, "interval_time = " + String.valueOf(interval));
    }
...
//				@Override
//				public void run() {
//					CobubLog.i(UTConstants.LOG_TAG, "Call onResume()");
//					if (usinglogManager == null)
...
//				@Override
//				public void run() {
//					CobubLog.i(UTConstants.LOG_TAG, "Call onPause()");
//					if (usinglogManager == null)
//						usinglogManager = new UsinglogManager(context);
...
//	}
    private static void updateContent(Context context) {
        UTAgent.contextWR = new WeakReference<Context>(context);
        context = null;
    }
}
