        ...
        try {
            ...
            for (int i = 1; i < dataarr.length; i++) {
                try {
                    ...
                    CobubLog.e(UTConstants.LOG_TAG, UploadActivityLog.class, "Message=" + e.getMessage());
                    continue;
                }
            }
            if (jsonarr.length() == 0) {
            ...
            // 发送之前再度判断
            if (CommonUtil.isNetworkAvailable(context)) {
                CobubLog.i(UTConstants.LOG_TAG, UploadActivityLog.class, "post activity info");
                MyMessage message = NetworkUtil.Post(UTConstants.BASE_URL
                        + UTConstants.USINGLOG_URL, postdata.toString());

                if (!message.isSuccess()) {
                    CobubLog.e(UTConstants.LOG_TAG, UploadActivityLog.class, message.getMsg());
