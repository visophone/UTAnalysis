...
class ErrorManager {
    ...
    private JSONObject prepareErrorJSON(String errorinfo) {
        ...
        try {
            ...
            errorInfo.put("lib_version", UTConstants.LIB_VERSION);

        } catch (JSONException e) {
            CobubLog.e(UTConstants.LOG_TAG, ErrorManager.class, e.toString());
            return null;
        }
        return errorInfo;
    }
    ...
    public void postErrorInfo(String error) {
        ...
        if (CommonUtil.getReportPolicyMode(context) == SendPolicy.POST_NOW
            ...
            MyMessage message = NetworkUtil.Post(UTConstants.BASE_URL
                    + UTConstants.ERROR_URL, postdata.toString());

            if (!message.isSuccess()) {
                CommonUtil.saveInfoToFile("errorInfo", errorJSON, context);
                CobubLog.e(UTConstants.LOG_TAG, ErrorManager.class, "Message=" + message.getMsg());
            }
        } else {
            CommonUtil.saveInfoToFile("errorInfo", errorJSON, context);
        }
