...
class ClientdataManager {
    ...
    JSONObject prepareClientdataJSON() throws JSONException {
        ...
        jsonClientdata.put("lib_version", UTConstants.LIB_VERSION);
    ...
    public void judgeSession(final Context context){
    	CobubLog.i(UTConstants.LOG_TAG,UsinglogManager.class, "judgeSession on clientdata");
        try {
            if (CommonUtil.isNewSession(context)) {
              String  session_id = CommonUtil.generateSession(context);
                CobubLog.i(UTConstants.LOG_TAG, UsinglogManager.class,"New Sessionid is " + session_id);
            }
        } catch (Exception e) {
    ...
    public void postClientData() {
        ...
        if (CommonUtil.isNetworkAvailable(context)) {
            MyMessage message = NetworkUtil.Post(UTConstants.BASE_URL
                    + UTConstants.CLIENTDATA_URL, postdata.toString());

            if (!message.isSuccess()) {
                CobubLog.e(UTConstants.LOG_TAG, ClientdataManager.class, "Error Code=" + message.getMsg());
