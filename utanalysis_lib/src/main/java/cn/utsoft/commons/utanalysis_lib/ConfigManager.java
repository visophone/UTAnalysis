    ...
    public void updateOnlineConfig() {
        ...
        if (CommonUtil.isNetworkAvailable(context)) {
            MyMessage message = NetworkUtil.Post(UTConstants.BASE_URL
                    + UTConstants.CONFIG_URL, jsonConfig.toString());
            try {
                ...
                if (isLocation == 0)
                    UTAgent.setAutoLocation(false);
                else
                    UTAgent.setAutoLocation(true);
                prefUtil.setValue("locationStatus", isLocation != 0);

                int isOnlyWifi = result_obj.getInt("updateOnlyWifi");// 只在wifi状态下更新
                if (isOnlyWifi == 0)
                    UTAgent.setUpdateOnlyWifi(false);
                else
                    UTAgent.setUpdateOnlyWifi(true);
                ...
                UTAgent.setSessionContinueMillis(context, session * 1000);

                // interval_time
                int interval_time = result_obj.getInt("intervalTime");// 获取到的数据单位为
                                                                  // 分钟 默认1分钟
                UTAgent.setPostIntervalMillis(context, interval_time);
