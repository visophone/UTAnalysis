...
class SaveInfo extends Thread {
    ...
    @Override
    public void run() {
        CobubLog.d(UTConstants.LOG_TAG, SaveInfo.class, "Save cache file " + filePath);
        CobubLog.d(UTConstants.LOG_TAG, SaveInfo.class, "json data " + arrObj.toString());
        if (arrObj.length() == 0) {
            return;
        }
            ...
            try {
                ...
                for (int i = 0; i < array.length(); i++) {
                    ...
                    try {
                        ...
                        writer.write(UTConstants.fileSep + jsonObject.toString());
                    } catch (JSONException e) {
                        CobubLog.e(UTConstants.LOG_TAG, e);
                    }
                }
