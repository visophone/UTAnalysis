...
class NetworkUtil {   
	...
	private static void initSSL() {
		CobubLog.d(UTConstants.LOG_TAG,NetworkUtil.class,"InitSSL start it:" + UTConstants.SDK_POS_NAME);
		System.setProperty("javax.net.ssl.keyStoreProvider",
				UTConstants.SDK_POS_NAME);
		System.setProperty("javax.net.ssl.certAlias",
				UTConstants.SDK_CSR_ALIAS);
		CobubLog.d(UTConstants.LOG_TAG,NetworkUtil.class,"InitSSL end it:" + UTConstants.SDK_CSR_ALIAS);
	}

	public static MyMessage Post(String url, String data) {
		CobubLog.d(UTConstants.LOG_TAG, NetworkUtil.class,"URL = " + url);
		CobubLog.d(UTConstants.LOG_TAG, NetworkUtil.class, "LENGTH:" + data.length() + " *Data = " + data + "*");

		if (!hasInitSSL && UTConstants.SDK_SECURITY_LEVEL.equals("2")) {
			initSSL();
			hasInitSSL = true;
		}
		 ...
		 * 3,测试：使用http
		*/
		if (UTConstants.SDK_SECURITY_LEVEL.equals("2")) {
			httpclient = new DefaultHttpClient(httpParams);
			// cpos with dn check
			if (!UTConstants.SDK_HTTPS_DN.equals("none")) {
				...
				try {
					...
					CobubLog.d(UTConstants.LOG_TAG,NetworkUtil.class, e.toString());
				}
			}
		} else if (UTConstants.SDK_SECURITY_LEVEL.equals("1")
		        && url.toLowerCase().startsWith("https")){
			// for https with company cert
			if (serverPort < 0) {
				serverPort = getPort();
			}
			CobubLog.d(UTConstants.LOG_TAG,NetworkUtil.class,"InitSSL port is:" + serverPort);
		...
		try {
			...
			CobubLog.d(UTConstants.LOG_TAG,NetworkUtil.class, "Status code="
			...
			CobubLog.d(UTConstants.LOG_TAG,NetworkUtil.class, "returnString = " + returnContent);
	...
	private static int getPort() {
		String url = UTConstants.BASE_URL.toLowerCase();
		CobubLog.d(UTConstants.LOG_TAG,NetworkUtil.class, url);
		int pos = url.indexOf(":");
